...
function setup() {
  ...
  elipses.push(createVector(3, 13));
  elipses.push(createVector(4, 10));
  elipses.push(createVector(6, 3));
  elipses.push(createVector(10, 13));
  elipses.push(createVector(13, 1));
...
function mostrarPantallaDeJuego() {
  ...
  if (ganaste) {
    ...
    for (let i = elipses.length - 1; i >= 0; i--) {
      let elipseX = elipses[i].x;
      let elipseY = elipses[i].y;
